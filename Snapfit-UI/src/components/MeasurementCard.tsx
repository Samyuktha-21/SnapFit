...
export default function MeasurementCard({
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-3xl border border-slate-800 bg-slate-950/70 p-6 shadow-xl backdrop-blur-sm overflow-hidden hover:border-slate-700 transition-all duration-300 group ${shadowClass}`}
    >
      ...
      {gaugePercentage !== undefined && (
        <div className="mt-5">
          <div className="flex items-center justify-between text-[10px] text-slate-500 font-bold mb-1.5 uppercase">
            <span>Accuracy Level</span>
            <span className="text-slate-300 font-mono font-semibold">{gaugePercentage}%</span>
          </div>
