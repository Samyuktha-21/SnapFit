...
// NOTE: auto height by credit-card reference used to live here as a Sobel
// bounding-box detector plus a single mm-per-pixel division. It was biased low
// by ~5% because it ignored that the card is held ~125mm in front of the body's
// own plane. It has been replaced by ./heightEstimator.js, which reconstructs
// the geometry properly. See the header there for the full explanation.
...
// HEIGHT alignment. Deliberately NOT the same as checkFrontAlignment: that one
// requires the wrists to be WIDER than the shoulders so the armpits are visible
// for the silhouette measurement, but the height capture needs the user holding
// a card flat against their chest, which puts a wrist well inside the shoulder
// span. Requiring both at once is unsatisfiable — the two poses genuinely
// conflict, so height gets its own capture step and its own checks.
export function checkHeightAlignment(lm) {
  const noseY = lm[LM.NOSE].y;
  const centerX = (lm[LM.L_SHOULDER].x + lm[LM.R_SHOULDER].x) / 2;
  const minVis = Math.min(
    ...[LM.L_SHOULDER, LM.R_SHOULDER, LM.L_HIP, LM.R_HIP, LM.L_ANKLE, LM.R_ANKLE]
  ...
  // The crown sits above the nose, and the whole body must be inside the frame:
  // a silhouette touching the top or bottom edge is clipped, and a clipped
  // silhouette silently under-measures rather than failing.
  const checks = {
    headInFrame: noseY > 0.07 && noseY < 0.22,
    feetInFrame: bottomY(lm) > 0.86 && bottomY(lm) < 0.99,
    centered: Math.abs(centerX - 0.5) < 0.12,
    upright: Math.abs(centerX - (lm[LM.L_ANKLE].x + lm[LM.R_ANKLE].x) / 2) < 0.10,
    confident: minVis > 0.6,
  };
  const reasons = [];
  if (!checks.headInFrame) reasons.push('Move your head into the top circle');
  if (!checks.feetInFrame) reasons.push('Step back — your whole body must be in frame');
  if (!checks.centered) reasons.push('Center yourself left-to-right');
  if (!checks.upright) reasons.push('Stand up straight, weight on both feet');
  if (!checks.confident) reasons.push('Improve lighting / face the camera');
